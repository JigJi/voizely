...
# Role/position/generic noun patterns — Gemini มักใช้แทนชื่อจริงเมื่อเดาไม่ได้
_ROLE_PATTERNS = _re.compile(
    r'(ผู้จัด|ผู้ประสาน|ผู้ดูแล|ผู้รับผิดชอบ|ผู้พัฒนา|ผู้ใช้|ผู้เข้าร่วม|ผู้บริหาร|ผู้อำนวยการ|'
    r'หัวหน้า|เจ้าหน้าที่|พนักงาน|ลูกน้อง|ลูกค้า|ทีม|ฝ่าย|แผนก|ผู้แทน|ผู้ช่วย|ผู้ประชุม|'
    r'นักพัฒนา|นักวิเคราะห์|นักวิชาการ|วิศวกร|ที่ประชุม|เจ้าของ|ประธาน|เลขา|'
    r'Manager|Developer|Engineer|Analyst|Lead|Director|Admin|Coordinator|Officer)'
)
...
def _clean_suggested_name(name: str) -> str | None:
    ...
    if not n:
        return None
    # Reject role/position/generic terms — suggestion ต้องเป็นชื่อจริง/ชื่อเล่นเท่านั้น
    if _ROLE_PATTERNS.search(n):
        return None
    return n
...
- สิ่งที่ต้องทำ: เป็น Markdown table มี 4 คอลัมน์: ลำดับ, รายละเอียด, กำหนดการ, ผู้รับผิดชอบ ผู้รับผิดชอบต้องเป็นชื่อ Speaker ตามที่ปรากฏก่อน : ใน transcript เท่านั้น (เช่น Speaker 1, Speaker 2) ห้ามใช้ชื่อจริง ชื่อเล่น หรือคำกว้างๆ เด็ดขาด เช่น "พี่ดาว" "ทีมพัฒนา" "ทีม" "ที่ประชุม" "เจ้าหน้าที่" ห้ามใส่ "TBC" ในคอลัมน์ผู้รับผิดชอบ ถ้าไม่ชัดว่าใครรับผิดชอบ ให้ใส่ Speaker ที่เป็นคนพูดมอบหมายงานนั้น กำหนดการต้องเป็นวันที่จริง (เช่น 04/04/2026) คำนวณจากวันที่ประชุมที่ให้ไว้ ห้ามใช้คำว่า "สัปดาห์นี้" "จันทร์หน้า" ถ้าไม่ระบุให้ใส่ TBC
...
- speaker_suggestions: suggested_name ต้องเป็น "ชื่อจริง" หรือ "ชื่อเล่น" ที่ปรากฏใน transcript เท่านั้น (เช่น จิ๊ก, เมย์, ปุ่ย, กนก, วิระพงศ์) โดย Speaker นั้นถูกเรียกชื่อ, แนะนำตัว, หรือคนอื่นเรียกถึง
  ห้ามใช้ตำแหน่งงาน, บทบาท, หรือคำทั่วไปเด็ดขาด เช่น "ผู้จัดการ", "ผู้ประสานงาน", "ผู้ดูแล", "หัวหน้า", "เจ้าหน้าที่", "ทีม", "ฝ่าย", "ผู้รับผิดชอบ", "Manager", "Developer"
  ถ้าไม่มีใครเรียกชื่อ Speaker นั้นใน transcript → ห้ามเดาจากบริบท/หน้าที่ → ห้ามใส่ suggestion นั้น
  ถ้าไม่มี Speaker คนไหนถูกเรียกชื่อเลย → ใส่ array ว่าง []
...
CHUNK_SUMMARY_PROMPT = """จากบทถอดความช่วงที่ {chunk_index} ของการประชุม (นาทีที่ {start_min}-{end_min}):

{transcript_chunk}

สรุปช่วงนี้เป็น JSON:
{{
  "topics": ["หัวข้อที่พูดถึงในช่วงนี้"],
  "details": ["- **หัวข้อ**: รายละเอียดสำคัญ 1-2 ประโยค"],
  "decisions": ["ข้อตัดสินใจในช่วงนี้ ถ้ามี"],
  "action_items": [{{"task": "งาน", "owner": "Speaker 1", "deadline": "TBC"}}],
  "speaker_suggestions": [{{"speaker": "Speaker 1", "suggested_name": "ชื่อจริง/ชื่อเล่นที่ปรากฏใน transcript", "reason": "เหตุผล"}}]
}}

กฎ:
- ตอบ JSON เท่านั้น ห้ามมีข้อความอื่น
- ห้ามเพิ่มข้อมูลที่ไม่มีใน transcript
- ใช้ชื่อ Speaker ตามที่ปรากฏก่อน : ใน transcript เท่านั้น (เช่น Speaker 1, Speaker 2) ห้ามใช้ชื่อจริง ชื่อเล่น หรือคำเรียกอื่นใด เช่น "พี่ดาว" "พี่นัท" "ทีม" "เจ้าหน้าที่" ห้ามใส่ "TBC" ถ้าไม่ชัดว่าใครรับผิดชอบ ให้ใส่ Speaker ที่เป็นคนพูดมอบหมายงานนั้น
- ห้ามขึ้นต้นด้วย "มีการ" "มีความ" "มีปัญหา"
- speaker_suggestions: ใส่ได้เฉพาะ Speaker ที่มีคนเรียก "ชื่อจริง/ชื่อเล่น" ชัดเจนใน transcript ห้ามใช้ตำแหน่งงาน/บทบาท/คำทั่วไป (เช่น "ผู้จัดการ", "ผู้ประสานงาน", "หัวหน้า", "ทีม") ถ้าไม่มีใครเรียกชื่อ → ใส่ []
- ถ้าไม่มีข้อมูลให้ใส่ array ว่าง"""

MERGE_PROMPT = """จากการสรุปแต่ละช่วงของการประชุมวันที่ {meeting_date} ต่อไปนี้:

{chunk_summaries}

รวมทั้งหมดเป็น MoM ฉบับสมบูรณ์ JSON:
{{
  "title": "ชื่อการประชุมสั้นๆ ภาษาไทยเท่านั้น ไม่เกิน 50 ตัวอักษร",
  "summary_short": "สรุป 1-2 ประโยค",
  "mom": "### สรุปภาพรวม\nสรุปใจความสำคัญและวัตถุประสงค์ของการประชุม 2-3 ประโยค\n\n### ประเด็นที่พูดคุย\n- **หัวข้อ 1**\n  - รายละเอียดข้อ 1\n  - รายละเอียดข้อ 2\n- **หัวข้อ 2**\n  - รายละเอียดข้อ 1\n\n### มติที่ประชุม\n- ข้อสรุป 1\n- ข้อสรุป 2\n\n### สิ่งที่ต้องทำ\n\n| ลำดับ | รายละเอียด | กำหนดการ | ผู้รับผิดชอบ |\n|------|-----------|---------|------------|\n| 1 | งาน 1 | TBC | Speaker 1 |\n| 2 | งาน 2 | TBC | Speaker 3 |",
  "sentiment": "positive|neutral|negative|mixed",
  "meeting_tone": "โทนการประชุมสั้นๆ",
  "meeting_type": "ประเภทการประชุม",
  "topics": ["หัวข้อรวม"],
  "action_items": [{{"task": "งาน", "owner": "Speaker 1", "deadline": "TBC"}}],
  "key_decisions": ["มติ"],
  "open_questions": ["คำถามค้าง"],
  "audio_quality": 85,
  "speaker_suggestions": [{{"speaker": "Speaker 1", "suggested_name": "ชื่อ", "reason": "เหตุผล"}}]
}}

กฎเหล็ก:
- ห้ามเพิ่มข้อมูลที่ไม่มีในสรุปแต่ละช่วง
- รวมหัวข้อที่ซ้ำกันเข้าด้วยกัน ตัดรายละเอียดซ้ำออก
- ประเด็นที่พูดคุย: สรุปเป็นหัวข้อใหญ่ แต่ละหัวข้อมี bullet ย่อย แต่ละ bullet เขียน 1-2 ประโยคกระชับตรงประเด็น บอกข้อเท็จจริงและรายละเอียดสำคัญ ถ้ารวมกลุ่มได้ให้รวม ไม่สำคัญให้ตัดออก ห้ามใส่ชื่อบุคคลใดทั้งสิ้น ใช้ "ทีม" หรือ "ที่ประชุม" แทน
  สำคัญมาก: ห้ามขึ้นต้นประโยคด้วยคำต่อไปนี้เด็ดขาด: "มีการ" "มีความ" "มีปัญหา" "มีข้อเสนอ" ถ้าเจอตัวเองกำลังจะเขียน "มี..." ให้หยุดแล้วเขียนใหม่
- มติที่ประชุม: เขียนเป็นข้อสรุปที่ตัดสินใจแล้ว ห้ามใส่ชื่อบุคคลใดทั้งสิ้น ห้ามขึ้นต้นด้วย "ทีมจะ" "จะมีการ" "จะดำเนินการ"
- สิ่งที่ต้องทำ: เป็น Markdown table มี 4 คอลัมน์ ผู้รับผิดชอบต้องเป็นชื่อ Speaker เท่านั้น (เช่น Speaker 1, Speaker 2, Speaker 3) ห้ามใช้ชื่อจริง ชื่อเล่น หรือคำกว้างๆ เด็ดขาด เช่น "พี่ดาว" "พี่นัท" "Tingle Platform" "ทีมพัฒนา" "ทีม" "ที่ประชุม" "ทีมผู้ใช้งาน" "เจ้าของรายงาน" "เจ้าหน้าที่" ห้ามใส่ "TBC" ในคอลัมน์ผู้รับผิดชอบ ถ้าไม่ชัดว่าใครรับผิดชอบ ให้ใส่ Speaker ที่เป็นคนพูดมอบหมายงานนั้น
- speaker_suggestions: suggested_name ต้องเป็น "ชื่อจริง/ชื่อเล่น" ที่ปรากฏใน transcript เท่านั้น ห้ามใช้ตำแหน่งงาน/บทบาท/คำทั่วไป (เช่น "ผู้จัดการ", "ผู้ประสานงาน", "หัวหน้า", "ทีม") รวมจากทุกช่วง deduplicate ถ้ามีหลาย suggestion สำหรับ Speaker เดียวกัน เลือกตัวที่มีเหตุผลดีที่สุด ถ้าไม่มีใครเรียกชื่อเลย → ใส่ []
- ตอบ JSON เท่านั้น ห้ามมีข้อความอื่น"""


def _call_gemini(prompt, max_tokens=8000, timeout=120):
    """Call Gemini API and return parsed JSON, with json_repair fallback."""
    payload = json.dumps({
        "model": "google/gemini-2.5-flash",
...
def _chunk_transcript(transcript_text, max_chars=20000):
    """Split transcript into chunks, breaking at speaker turn boundaries."""
...
def generate_analysis_chunked(transcript_text, custom_instructions=None, meeting_date=None):
    """Chunked analysis for long transcripts (1hr+)."""
    if not meeting_date:
        from datetime import datetime, timezone, timedelta
        meeting_date = datetime.now(timezone(timedelta(hours=7))).strftime("%d/%m/%Y")

    chunks = _chunk_transcript(transcript_text, max_chars=20000)
    logger.info("Chunked analysis: %d chunks from %d chars", len(chunks), len(transcript_text))
    ...
    for i, chunk in enumerate(chunks):
        ...
        start_min = int(lines_so_far / total_lines * 100)  # rough % as proxy
        end_min = int((lines_so_far + chunk_lines) / total_lines * 100)
        lines_so_far += chunk_lines
        ...
        try:
            result = _call_gemini(prompt, max_tokens=4000)
            chunk_results.append(result)
            logger.info("Chunk %d/%d summarized: %d topics", i + 1, len(chunks), len(result.get("topics", [])))
        except Exception as e:
            logger.warning("Chunk %d/%d failed: %s", i + 1, len(chunks), e)
            chunk_results.append({"topics": [], "details": [], "decisions": [], "action_items": []})

    # Step 2: Merge all chunk summaries
    summaries_text = ""
    for i, cr in enumerate(chunk_results):
        summaries_text += f"\n--- ช่วงที่ {i+1} ---\n"
        summaries_text += f"หัวข้อ: {', '.join(cr.get('topics', []))}\n"
        for d in cr.get("details", []):
            summaries_text += f"{d}\n"
        if cr.get("decisions"):
            summaries_text += f"มติ: {', '.join(cr.get('decisions', []))}\n"
        if cr.get("action_items"):
            for ai in cr["action_items"]:
                summaries_text += f"งาน: {ai.get('task', '')} ({ai.get('owner', 'TBC')})\n"
        if cr.get("speaker_suggestions"):
            for ss in cr["speaker_suggestions"]:
                summaries_text += f"เดาชื่อ: {ss.get('speaker', '')} = {ss.get('suggested_name', '')} ({ss.get('reason', '')})\n"
    ...
    try:
        final = _call_gemini(merge_prompt, max_tokens=8000)
        logger.info("Merge complete: title=%s, mom_len=%d", final.get("title", ""), len(final.get("mom", "")))
        return final
    except Exception as e:
...
def generate_analysis(transcript_text, custom_instructions=None, meeting_date=None):
    """Generate MoM + sentiment + meeting type. Auto-chunks for long transcripts."""
    # Long transcripts: use chunked approach
    if len(transcript_text) > 40000:
        logger.info("Transcript too long (%d chars), using chunked analysis", len(transcript_text))
        return generate_analysis_chunked(transcript_text, custom_instructions, meeting_date)
    ...
    # Scale max_tokens based on transcript length to avoid truncation
    transcript_len = len(transcript_text)
    if transcript_len < 30000:
        max_tokens = 8000
    elif transcript_len < 50000:
        max_tokens = 16000
    else:
        max_tokens = 32000
    payload = json.dumps({
    ...
    try:
        ...
        # Try json_repair first (handles unescaped quotes, trailing commas, etc.)
        try:
            from json_repair import repair_json
            repaired = repair_json(raw, return_objects=True)
            if isinstance(repaired, dict):
                logger.info("json_repair result keys: %s, mom length: %d", list(repaired.keys()), len(repaired.get("mom", "")))
                if repaired.get("mom") or repaired.get("title"):
                    return repaired
